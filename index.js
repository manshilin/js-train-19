...
if(Object.getPrototypeOf(Biography) !== Novel) {
 ...
 * Об'єкт: ScienceBook
 * Властивості та функції наслідуються від об'єкта Book
 * Також тут використовується інкапсуляція для створення властивості 'info', яка не може бути
 *  змінена напряму, а лише змінюється за допомогю гетера
 */
// Створюємо об'єкт ScienceBook, наслідуємо властивості і функції від об'єкта Book
const ScienceBook = Object.create(Book);
// Додаємо властивість 'info' за допомогою Object.defineProperty
// Зробимо щоб 'info' не можно було видалити або змінити, перевіримо і спробуємо присвоїти 
//ій будь яке значення (це потрібно робити ззовні defineProperty),
Object.defineProperty(ScienceBook, 'info', {
    value: "написана в 1915 році",
    writable: false,
    configurable: false
});
// Спробуємо присвоїти значення властивості info
// Виведе повідомлення про відхилення
try {
    ScienceBook.info = "написана в 1916 році";
} catch (Error) {  
    console.log(Error.message, "властивість не змінена");
}

// ??????????????????????????????????

// Використовуємо сетер (задаємо нове значення)
 // Виведе повідомлення про встановлення нового значення

// Використовуємо гетер
...
Object.defineProperty(Textbook, 'info', {
